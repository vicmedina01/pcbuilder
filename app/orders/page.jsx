...
export default async function OrdersPage() {
  ...
  if (!session?.user?.email) {
    return (
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-10 md:px-8">
        ...
        <h1 className="mt-3 text-4xl font-black text-white">Sign in to view your orders</h1>
  ...
  return (
    <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 md:px-8">
      ...
      {orders.length === 0 ? (
        <section className="border border-white/10 bg-[#111412] p-6">
          ...
          <Link href="/products" className="mt-5 inline-block bg-[#b7f34a] px-5 py-3 font-black uppercase text-black hover:bg-[#93d329]">
            Browse products
          </Link>
        </section>
      ) : (
        <div className="grid gap-5">
          {orders.map((order) => (
            <article key={order.id} className="border border-white/10 bg-[#111412] p-5">
              <div className="flex flex-col gap-3 border-b border-white/10 pb-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap gap-3 text-sm">
                  <span className="border border-[#b7f34a]/30 px-3 py-1 font-semibold text-[#dfffa9]">{order.status}</span>
                  <span className="border border-white/10 px-3 py-1 text-gray-300">{formatDate(order.createdAt)}</span>
                </div>
              </div>

              <div className="mt-4 grid gap-3">
                {order.items.map((item) => (
                  <div key={item.id} className="flex flex-col gap-2 border border-white/10 bg-[#090b0a] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
